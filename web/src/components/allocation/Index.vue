<template>
    <div class="wares-table">
        <section>
            <b-field>
                <b-button tag="router-link" to="/allocation/new" type="is-success">Создать новый</b-button>
            </b-field>
            <b-table
                :data="data"
                :loading="loading"
            ...
            >
                    ...
                    <b-table-column field="status" label="Статус" v-slot="props">
                        <b-tag :type="props.row.status ? 'is-success' : 'is-warning'">{{props.row.status ? 'проведен' : 'не проведен'}}</b-tag>
                    </b-table-column>
                    <b-table-column field="actions" label="Действия" v-slot="props">
                        <div class="buttons">
                        <router-link :to="'/allocation/' + props.row.id" class="button is-info is-small">Редактирование</router-link>
                        <b-button v-if="props.row.status === 0" type="is-danger" @click="deleteRow(props.row.id)" size="is-small">Удалить</b-button>
                        <b-button v-else type="is-warning" @click="deleteRow(props.row.id)" size="is-small">В архив</b-button>
                        </div>
                    </b-table-column>
            </b-table>
        </section>
...
export default {
    ...
    methods: {
        ...
        deleteRow: async function (id) {
            await Axios.delete('/allocation/' + id);
            this.loadAsyncData();
        },
